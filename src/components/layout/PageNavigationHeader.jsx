...
const PageNavigationHeader = ({
  ...
  return (
    <section
      id={id}
      className="relative h-100 overflow-hidden flex-center"
      style={{
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto text-center space-y-4">
        <h1
          ref={titleRef}
          className="text-5xl md:text-7xl font-bold text-[var(--accent-yellow)]"
        >
          <span className="text-white">{h1}</span> {h1Yellow}
        </h1>

        <p ref={pRef} className="max-w-2xl mx-auto text-[var(--muted-gray)]">
          {p}
        </p>

        <div ref={routeRef} className="text-white">
          <ul className="flex-center gap-1 w-full max-w-md mx-auto">
            {breadcrumbs
              .map((crumb, index) => (
                <li
                  key={`crumb-${index}`}
